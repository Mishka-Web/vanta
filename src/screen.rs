...
pub struct Cell { ch: char }
impl Default for Cell { fn default() -> Self { Self { ch: ' ' } } }
impl Cell { pub fn new(ch: char) -> Self { Self { ch } } pub fn ch(self) -> char { self.ch } }
...
impl Screen {
    pub fn new(width: u16, height: u16) -> Self {
        ...
        Self { width, height, cells: vec![Cell::default(); width as usize * height as usize], cursor_row: 0, cursor_col: 0 }
    }
    pub fn width(&self) -> u16 { self.width }
    pub fn height(&self) -> u16 { self.height }
    pub fn cursor_row(&self) -> u16 { self.cursor_row }
    pub fn cursor_col(&self) -> u16 { self.cursor_col }
...
mod tests {
    use super::*;
    #[test] fn screen_starts_with_requested_size() { let s = Screen::new(80,24); assert_eq!(s.cells().len(),80*24); }
    #[test] fn write_str_writes_cells() { let mut s=Screen::new(10,2); s.write_str(0,0,"VANTA"); let t:String=s.cells()[0..5].iter().map(|c|c.ch()).collect(); assert_eq!(t,"VANTA"); }
    #[test] fn set_cursor_is_clamped_to_screen() { let mut s=Screen::new(5,3); s.set_cursor(99,99); assert_eq!((s.cursor_row(),s.cursor_col()),(2,4)); }
}
