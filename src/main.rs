#![cfg_attr(not(windows), allow(dead_code))]
#[cfg(windows)]
mod input;
#[cfg(windows)]
mod screen;
#[cfg(windows)]
mod terminal;
#[cfg(windows)]
use input::{KeyCode, KeyEvent};
#[cfg(windows)]
use screen::Screen;
#[cfg(windows)]
use std::io;

#[cfg(windows)]
fn main() -> io::Result<()> {
    let _terminal = terminal::Terminal::enter()?;
    let (width, height) = terminal::terminal_size()?;
    let mut screen = Screen::new(width, height);
    draw_ui(&mut screen, None);
    terminal::render(&screen)?;
    loop {
        let key = input::read_key()?;
        if matches!(key.code, KeyCode::Char('q')) && key.modifiers.is_empty() {
            break;
        }
        match key.code {
            KeyCode::Left => screen.move_cursor_left(),
            KeyCode::Right => screen.move_cursor_right(),
            KeyCode::Up => screen.move_cursor_up(),
            KeyCode::Down => screen.move_cursor_down(),
            _ => {}
        }
        let (new_width, new_height) = terminal::terminal_size()?;
        if new_width != screen.width() || new_height != screen.height() {
            screen.resize(new_width, new_height);
        }
        draw_ui(&mut screen, Some(key));
        terminal::render(&screen)?;
    }
    Ok(())
}
#[cfg(windows)]
fn draw_ui(screen: &mut Screen, last_key: Option<KeyEvent>) {
    screen.clear();
    screen.write_str(0, 0, "VANTA v0.0.3");
    screen.write_str(2, 0, "Input Engine");
    screen.write_str(4, 0, "Arrows move the virtual cursor.");
    screen.write_str(5, 0, "Press q to quit.");
    if let Some(key) = last_key {
        screen.write_str(7, 0, &format!("Code: {:?}", key.code));
        screen.write_str(
            8,
            0,
            &format!(
                "Modifiers: shift={} ctrl={} alt={} repeat={}",
                key.modifiers.shift(),
                key.modifiers.ctrl(),
                key.modifiers.alt(),
                key.repeat
            ),
        );
    }
    let status = format!(
        "size={}x{}  cursor={},{}",
        screen.width(),
        screen.height(),
        screen.cursor_col(),
        screen.cursor_row()
    );
    let row = screen.height().saturating_sub(1);
    screen.write_str(row, 0, &status);
}
#[cfg(not(windows))]
fn main() {
    eprintln!("VANTA v0.0.3 currently targets Windows only.");
}
